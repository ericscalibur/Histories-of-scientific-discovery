...
<p>The plan: photograph the star field during totality, then photograph the <i>same stars</i> months later at night, when the Sun was elsewhere. Lay the glass plates side by side, measure with microscopes. If Einstein was right, the stars near the Sun's edge would sit <i>visibly out of place</i> — pushed outward by the Sun's dent in space.</p>""",
            cps=[dict(
                type="num",
                kicker="You are the measuring machine",
                q="""Try the astronomers' job yourself (shifts exaggerated about 1,000&times; so your eyes can do what their microscopes did). The night plate's star positions are laid over the eclipse plate as small <b>gray crosses</b>. An honest star sits exactly on its cross. <b>Click every star on the eclipse plate that has shifted</b>, then press <i>Compare plates</i> &mdash; or count the culprits and type the number.""",
                figure='<div style="display:flex; gap:14px; flex-wrap:wrap; justify-content:center; margin:10px 0;">\n  <div style="text-align:center;">\n    <svg id="es-night" width="300" height="212" viewBox="0 0 340 240" style="background:#04070f; border:1px solid #3a4a86; border-radius:8px; max-width:100%;">\n      <circle cx="55" cy="45" r="4" fill="#fff"/><circle cx="285" cy="55" r="4" fill="#fff"/>\n      <circle cx="125" cy="95" r="4" fill="#fff"/><circle cx="215" cy="150" r="4" fill="#fff"/>\n      <circle cx="160" cy="185" r="4" fill="#fff"/><circle cx="45" cy="190" r="4" fill="#fff"/>\n      <circle cx="295" cy="205" r="4" fill="#fff"/>\n    </svg>\n    <div style="font-size:.78rem; color:#aebadf; margin-top:4px;">NIGHT PLATE &middot; same stars, months earlier</div>\n  </div>\n  <div style="text-align:center;">\n    <svg id="es-day" width="300" height="212" viewBox="0 0 340 240" style="background:#04070f; border:1px solid #3a4a86; border-radius:8px; max-width:100%;">\n      <circle cx="170" cy="118" r="40" fill="#e8a90c" opacity="0.13"/>\n      <circle cx="170" cy="118" r="32" fill="#e8a90c" opacity="0.2"/>\n      <circle cx="170" cy="118" r="26" fill="#0a0d14" stroke="#ffe9b0" stroke-width="1.5"/>\n      <text x="170" y="122" text-anchor="middle" font-size="9" fill="#8f9cc4">eclipsed sun</text>\n      <g stroke="#5a6a9a" stroke-width="1.2">\n        <path d="M48 45h14M55 38v14"/><path d="M278 55h14M285 48v14"/>\n        <path d="M118 95h14M125 88v14"/><path d="M208 150h14M215 143v14"/>\n        <path d="M153 185h14M160 178v14"/><path d="M38 190h14M45 183v14"/>\n        <path d="M288 205h14M295 198v14"/>\n      </g>\n      <g id="es-stars">\n        <circle data-star="A" data-shift="0" cx="55" cy="45" r="4" fill="#fff"/>\n        <circle data-star="B" data-shift="0" cx="285" cy="55" r="4" fill="#fff"/>\n        <circle data-star="C" data-shift="1" cx="118" cy="91" r="4" fill="#fff"/>\n        <circle data-star="D" data-shift="1" cx="221" cy="154" r="4" fill="#fff"/>\n        <circle data-star="E" data-shift="1" cx="159" cy="191" r="4" fill="#fff"/>\n        <circle data-star="F" data-shift="0" cx="45" cy="190" r="4" fill="#fff"/>\n        <circle data-star="G" data-shift="0" cx="295" cy="205" r="4" fill="#fff"/>\n      </g>\n      <g id="es-hits"></g>\n    </svg>\n    <div style="font-size:.78rem; color:#aebadf; margin-top:4px;">ECLIPSE PLATE &middot; May 29, 1919 &middot; gray crosses = night positions laid on top</div>\n  </div>\n</div>\n<div style="text-align:center; margin:4px 0 10px;">\n  <button type="button" id="es-compare" style="background:#e8a90c; color:#2a2005; font-weight:bold; font-size:1rem; border:none; border-radius:8px; padding:10px 20px; font-family:Georgia,serif; cursor:pointer;">Compare plates &#128269;</button>\n  <div id="es-msg" style="margin-top:8px; font-size:.95rem; min-height:1.2em; color:#ffd97a;"></div>\n</div>\n<script>\n(function(){\n  var sel = {};\n  var svg = document.getElementById(\'es-day\');\n  if (!svg) return;\n  var stars = svg.querySelectorAll(\'#es-stars circle\');\n  var hits = document.getElementById(\'es-hits\');\n  stars.forEach(function(st){\n    var pad = document.createElementNS(\'http://www.w3.org/2000/svg\',\'circle\');\n    pad.setAttribute(\'cx\', st.getAttribute(\'cx\'));\n    pad.setAttribute(\'cy\', st.getAttribute(\'cy\'));\n    pad.setAttribute(\'r\', \'13\');\n    pad.setAttribute(\'fill\', \'transparent\');\n    pad.style.cursor = \'pointer\';\n    var ring = document.createElementNS(\'http://www.w3.org/2000/svg\',\'circle\');\n    ring.setAttribute(\'cx\', st.getAttribute(\'cx\'));\n    ring.setAttribute(\'cy\', st.getAttribute(\'cy\'));\n    ring.setAttribute(\'r\', \'9\');\n    ring.setAttribute(\'fill\', \'none\');\n    ring.setAttribute(\'stroke\', \'#e8a90c\');\n    ring.setAttribute(\'stroke-width\', \'2\');\n    ring.setAttribute(\'visibility\', \'hidden\');\n    hits.appendChild(ring);\n    hits.appendChild(pad);\n    var id = st.getAttribute(\'data-star\');\n    pad.addEventListener(\'click\', function(){\n      if (svg.dataset.done) return;\n      sel[id] = !sel[id];\n      ring.setAttribute(\'visibility\', sel[id] ? \'visible\' : \'hidden\');\n    });\n  });\n  document.getElementById(\'es-compare\').addEventListener(\'click\', function(){\n    if (svg.dataset.done) return;\n    var msg = document.getElementById(\'es-msg\');\n    var right = 0, wrong = 0, missed = 0;\n    stars.forEach(function(st){\n      var id = st.getAttribute(\'data-star\'), shifted = st.getAttribute(\'data-shift\') === \'1\';\n      if (sel[id] && shifted) right++;\n      else if (sel[id] && !shifted) wrong++;\n      else if (!sel[id] && shifted) missed++;\n    });\n    if (wrong === 0 && missed === 0 && right > 0){\n      svg.dataset.done = \'1\';\n      msg.style.color = \'#7fe0a7\';\n      msg.textContent = \'\\u2713 All three found \\u2014 the stars nearest the Sun, pushed outward. You just did what the measuring machines did.\';\n      var input = document.getElementById(\'a3\');\n      if (input && window.check){ input.value = String(right); window.check(3); }\n    } else if (right === 0 && wrong === 0) {\n      msg.style.color = \'#ffd97a\';\n      msg.textContent = \'Click the suspicious stars on the eclipse plate first \\u2014 a shifted star sits OFF its gray cross.\';\n    } else {\n      msg.style.color = \'#ff9d9d\';\n      msg.textContent = \'\\u2717 Not quite \\u2014 \' + (wrong ? wrong + \' innocent star(s) accused\' : \'\') +\n        (wrong && missed ? \', \' : \'\') + (missed ? missed + \' shifted star(s) still hiding\' : \'\') +\n        \'. Compare each star with its gray cross, and watch the neighborhood of the Sun.\';\n    }\n  });\n})();\n</script>',
                answers=[3], unit="shifted stars",
                hint="A shifted star sits noticeably OFF its gray cross &mdash; and the culprits huddle near the Sun, where the dent in space is deepest. (The real shifts were about 1/2000 of a degree; no wonder they needed microscopes.)",
            )],
        ),
        dict(
